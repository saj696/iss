...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <?= $this->Html->link(__('Stock Logs'), ['action' => 'index']) ?>
            <i class="fa fa-angle-right"></i>
        </li>
        <li><?= __('Reduce Stock') ?></li>
    </ul>
</div>
    ...
    <div class="col-md-12">
        <div class="portlet box blue-hoki">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-plus-square-o fa-lg"></i><?= __('Change Stock') ?>
                </div>
                <div class="tools">
            ...
            <div class="portlet-body">
                <?= $this->Form->create($stockLogs, ['class' => 'form-horizontal', 'role' => 'form']) ?>
                <div class="row">
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '.store', function() {
            ...
            if(store>0)
            {
                $.ajax({
                    type: 'POST',
                    url: '<?= $this->Url->build("/StockLogs/ajax")?>',
                    data: {store: store},
        ...
        $(document).on('change', '.item', function() {
            ...
            else
            {
                if(stock_id>0)
                {
                    $.ajax({
                        type: 'POST',
                        url: '<?= $this->Url->build("/StockLogs/existing")?>',
                        data: {stock_id: stock_id, store_id:store_id},
                        success: function (data, status) {
                            //console.log(data);
