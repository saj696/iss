...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li><?= $this->Html->link(__('Customer Performance Report'), ['action' => 'index']) ?></li>
    </ul>
</div>
    ...
    <div class="col-md-12">
        <div class="portlet box grey-cascade">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-plus-square-o fa-lg"></i><?= __('Customer Performance Report') ?>
                </div>
            </div>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('change', '.explore_level', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build("/ReportCustomerPerformances/ajax/explore_units")?>',
        ...
        $(document).on('change', '.explore_unit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build("/ReportCustomerPerformances/ajax/display_units")?>',
        ...
        $(document).on('change', '.unit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build("/ReportCustomerPerformances/ajax/customers")?>',
                data: {unit:unit},
                success: function (data, status) {
                    obj.closest('.input').next().find('.col-sm-9').html('');
                    obj.closest('.input').next().find('.col-sm-9').html(data);
