...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li><?= $this->Html->link(__('PO Report'), ['action' => 'index']) ?></li>
    </ul>
</div>
    ...
    <div class="col-md-12">
        <div class="portlet box grey-cascade">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-plus-square-o fa-lg"></i><?= __('PO Report') ?>
                </div>
            </div>
            ...
            <div class="portlet-body">
                ...
                <div class="row">
                    <div class="col-md-7 col-md-offset-2">
                        ...
                        echo $this->Form->input('po_status', ['options'=>Configure::read('po_status'), 'class'=>'form-control po_status', 'empty'=>'Select', 'required'=>'required']);
                        ...
                        echo $this->Form->input('customer_id', ['empty'=>'Select', 'class'=>'form-control customer']);
                        ?>
                    </div>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('change', '.explore_level', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build("/ReportPos/ajax/parent_units")?>',
        ...
        $(document).on('change', '.parent_unit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build("/ReportPos/ajax/units")?>',
        ...
        $(document).on('change', '.unit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build("/ReportPos/ajax/customers")?>',
                data: {unit:unit},
                success: function (data, status) {
                    obj.closest('.input').next().find('.col-sm-9').html('');
                    obj.closest('.input').next().find('.col-sm-9').html(data);
