...
class ReportProductWiseMonthlySalesController extends AppController
{
    ...
    public function index()
    {
        ...
        $this->set(compact('parentLevels', 'years'));
    }
    ...
    public function loadReport($param)
    {
        ...
        if ($this->request->is('post')) {
            ...
            $monthEndArray = Configure::read('month_end');

            foreach($monthEndArray as $month=>$end){
                $start_date = strtotime('01-'.$month.'-'.$year);
                $end_date = strtotime($end.'-'.$month.'-'.$year);
                ...
                $sales[$month] = $stmt->fetchAll('assoc');
            }

            $revisedArr = [];
            foreach($sales as $month=>$detail){
                foreach($detail as $info){
                    $revisedArr[$info['ITEM_NAME'].'-'.$info['unit_size'].'-'.$info['unit_type']][$month] = $info;
                }
            }

            $sales = $revisedArr;
            $this->set('sales', $revisedArr);
        }
        if ($param == 'report') {
