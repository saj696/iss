 ...
 */
class ReportCustomerPerformancesController extends AppController
{
    /**
     * Index method
     *
    ...
    public function loadReport($param)
    {
        if ($this->request->is(['post', 'get'])) {
            ...
            $this_month = date('m');
            if($this_month>=7){
                $december_closing_start_date = strtotime('01-07-'.date('Y'));
                $december_closing_end_date = strtotime('31-12-'.date('Y'));
            }else{
                $december_closing_start_date = strtotime('01-07-'.(date('Y')-1));
                $december_closing_end_date = strtotime('31-12-'.(date('Y')-1));
            }
